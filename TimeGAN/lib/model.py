...
class Generator(nn.Module):
    ...
    def forward(self, input, labels, sigmoid=False):
        """
        Args:
          input: latent input tensor (L, N, z_dim)
          labels: tensor (N,) com índices inteiros das classes
        """
        labels_emb = self.label_embedding(labels)  # (N, label_embed_dim)
        labels_emb = labels_emb.view(self.batch_size, self.seq_len, -1)
...
class Discriminator(nn.Module):
  """Discriminate the original and synthetic time-series data and classify the sequence.
  ...
  Returns:
    - Y_hat: classification results between original and synthetic time-series (real/fake)
    - class_logits: logits for class prediction (multi-class)
  """
  def __init__(self, opt):
    super(Discriminator, self).__init__()
    self.rnn = nn.GRU(input_size=opt.hidden_dim, hidden_size=opt.hidden_dim, num_layers=opt.num_layer)
    self.fc = nn.Linear(opt.hidden_dim, opt.hidden_dim)
    self.fc_cls = nn.Linear(opt.hidden_dim * opt.seq_len, opt.num_classes)  # Class head
    self.sigmoid = nn.Sigmoid()
    self.apply(_weights_init)
  ...
  def forward(self, input, sigmoid=True):
    ...
    d_cls = d_outputs.view(d_outputs.shape[0], -1)
    Y_cls = self.fc_cls(d_cls)
    if sigmoid:
