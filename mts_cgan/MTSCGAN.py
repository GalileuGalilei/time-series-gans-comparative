...
class Generator(nn.Module):
    def __init__(self, seq_len=150, patch_size=15, channels=3, context_shape = 1, num_classes=9, latent_dim=100, embed_dim=10, depth=3,
                 num_heads=5, forward_drop_rate=0.5, attn_drop_rate=0.5):
        super(Generator, self).__init__()
        self.channels = channels #cgan: channel of the timeseries generated (1)
    ...
    def forward(self, z, context):
        ...
        transp_cat_l = self.l4(transp_cat)                               
        cat_1d = torch.transpose(transp_cat_l,1,2)                        
        x = self.l3(cat_1d).view(-1, self.seq_len, self.embed_dim)  
