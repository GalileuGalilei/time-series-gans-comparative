...
def load_model_generator(seq_len, num_channels, num_classes, model_path):
    # Load the model
    gen_net = Generator(seq_len=seq_len, channels=num_channels, num_classes=num_classes, latent_dim=32, data_embed_dim=24,
...
def generate_fake_samples(gen_net, fake_labels):
    ...
    fake_noise = torch.FloatTensor(np.random.normal(0, 1, (new_samples, 32)))
